...
var pxsim;
(function (pxsim) {
    var ArMarker = (function () {
        function ArMarker(value) {
            this.value = value;
        }
        return ArMarker;
    }());
    pxsim.ArMarker = ArMarker;
})(pxsim || (pxsim = {}));
var pxsim;
(function (pxsim) {
    var colors;
    ...
    })(colors = pxsim.colors || (pxsim.colors = {}));
})(pxsim || (pxsim = {}));
var pxsim;
(function (pxsim) {
    var design;
    (function (design) {
        ...
        function setText(marker, text) {
            var m = pxsim.board().marker(marker);
            var billboardMesh = pxsim.three.createBillboard(marker, m.color());
            var textMesh = pxsim.three.createText(text, m.fontColor(), marker);
            var group = m.group();
            ...
            pxsim.three.removeShapeFromMarker(marker);
            pxsim.three.removeTextFromMarker(marker);
            group.add(billboardMesh);
            group.add(textMesh);
        }
        design.setText = setText;
        /**
        ...
        //% weight=96
        //% blockNamespace=design inBasicCategory=true
        function setNumber(marker, number) {
            var m = pxsim.board().marker(marker);
            var billboardMesh = pxsim.three.createBillboard(marker, m.color());
            var textMesh = pxsim.three.createText(number.toString(), m.fontColor(), marker);
            billboardMesh.name = marker.toString() + '-shape';
            billboardMesh.rotation.x = Math.PI / 2;
            textMesh.name = marker.toString() + '-text';
            textMesh.rotation.x = -Math.PI / 2;
        ...
        //% blockNamespace=design inBasicCategory=true
        function setShape(marker, shape) {
            var m = pxsim.board().marker(marker);
            var geometry = pxsim.three.createGeometry(shape);
        ...
        //% weight=20
        //% blockNamespace=design inBasicCategory=true
        function setModel(marker, type, content) {
            var m = pxsim.board().marker(marker);
            var mod = pxsim.three.loadModel(type, content);
            var material = new THREE.MeshPhongMaterial({
                transparent: true,
                opacity: m.opacity(),
                color: m.color(),
            ...
            var boundingBox = new THREE.Box3().setFromObject(mod);
            var maxDimension = Math.max(boundingBox.max.x, boundingBox.max.y, boundingBox.max.z);
            var autoScale = 1;
            if (maxDimension > 1)
                autoScale = maxDimension / 1.25;
            else if (maxDimension < 0.5)
                autoScale = maxDimension * 2;
        ...
        //% weight=99
        //% blockNamespace=design inBasicCategory=true
        function setColor(marker, color) {
            var m = pxsim.board().markers[marker.toString()];
            m.setColor(color);
            var object = m.shapeObject();
            if (object) {
        ...
        //% weight=95
        //% blockNamespace=design inBasicCategory=true
        function setOpacity(marker, value) {
            var m = pxsim.board().markers[marker.toString()];
            m.setOpacity(value);
            var object = m.shapeObject();
            if (object) {
        ...
        //% weight=97
        //% blockNamespace=design inBasicCategory=true
        function setTextColor(marker, color) {
            var m = pxsim.board().marker(marker);
            m.setFontColor(color);
            var object = m.textObject();
            if (object)
        ...
        function setScale(marker, size) {
            var m = pxsim.board().marker(marker);
            m.setScale(size);
            var object = m.shapeObject();
            if (object)
                object.scale.set(m.scaleX(), m.scaleY(), m.scaleZ());
        }
        design.setScale = setScale;
        /**
        ...
        //% inlineInputMode="inline"   
        function setScale3D(marker, x, y, z) {
            var m = pxsim.board().marker(marker);
            m.setScale(x, y, z);
            var object = m.shapeObject();
            if (object)
        ...
        //% blockNamespace=design inBasicCategory=true
        //% inlineInputMode="inline"    
        function setPosition(marker, x, y, z) {
            var m = pxsim.board().marker(marker);
            m.setPosition(x, y, z);
            var object = m.shapeObject();
            if (object) {
        ...
        design.setPosition = setPosition;
        /**
        ...
        //% blockNamespace=design inBasicCategory=true
        //% inlineInputMode="inline"    
        function setRotation(marker, degrees) {
            var m = pxsim.board().marker(marker);
        ...
        design.setRotation = setRotation;
        /**
        ...
        //% blockNamespace=design advanced=true
        //% inlineInputMode="inline"    
        function setRotation3D(marker, x, y, z) {
            var m = pxsim.board().marker(marker);
            m.setRotation(x, y, z);
            var object = m.shapeObject();
            if (object) {
        ...
        design.setRotation3D = setRotation3D;
        /**
        ...
        design.removeFilter = removeFilter;
        function addFilterHelper(filter) {
            var canvas = document.getElementsByTagName("canvas")[0];
            var video = document.getElementsByTagName("video")[0];
            var filterString = canvas.style.filter;
            if (canvas && video) {
        ...
        design.addFilterHelper = addFilterHelper;
        function removeFilterHelper(filter) {
            var canvas = document.getElementsByTagName("canvas")[0];
            var video = document.getElementsByTagName("video")[0];
            if (canvas && video) {
                var filterString = canvas.style.filter;
        ...
        design.removeFilterHelper = removeFilterHelper;
        function removeAllFilters() {
            removeFilter(1 /* Grayscale */);
            removeFilter(3 /* Invert */);
            removeFilter(4 /* Saturate */);
            removeFilter(5 /* HueRotate */);
            removeFilter(6 /* Blur */);
            removeFilter(7 /* Contrast */);
            removeFilter(8 /* Green */);
            removeFilter(2 /* Sepia */);
        }
        design.removeAllFilters = removeAllFilters;
    })(design = pxsim.design || (pxsim.design = {}));
})(pxsim || (pxsim = {}));
var font;
(function (font) {
    font.helvetiker_regular = { "glyphs": { "ο": { "x_min": 0, "x_max": 712, "ha": 815, "o": "m 356 -25 q 96 88 192 -25 q 0 368 0 201 q 92 642 0 533 q 356 761 192 761 q 617 644 517 761 q 712 368 712 533 q 619 91 712 201 q 356 -25 520 -25 m 356 85 q 527 175 465 85 q 583 369 583 255 q 528 562 583 484 q 356 651 466 651 q 189 560 250 651 q 135 369 135 481 q 187 177 135 257 q 356 85 250 85 " }, "S": { "x_min": 0, "x_max": 788, "ha": 890, "o": "m 788 291 q 662 54 788 144 q 397 -26 550 -26 q 116 68 226 -26 q 0 337 0 168 l 131 337 q 200 152 131 220 q 384 85 269 85 q 557 129 479 85 q 650 270 650 183 q 490 429 650 379 q 194 513 341 470 q 33 739 33 584 q 142 964 33 881 q 388 1041 242 1041 q 644 957 543 1041 q 756 716 756 867 l 625 716 q 561 874 625 816 q 395 933 497 933 q 243 891 309 933 q 164 759 164 841 q 325 609 164 656 q 625 526 475 568 q 788 291 788 454 " },
            ...
            "g": { "x_min": 0, "x_max": 686, "ha": 838, "o": "m 686 34 q 586 -213 686 -121 q 331 -306 487 -306 q 131 -252 216 -306 q 31 -84 31 -190 l 155 -84 q 228 -174 166 -138 q 345 -207 284 -207 q 514 -109 454 -207 q 564 89 564 -27 q 461 6 521 36 q 335 -23 401 -23 q 88 100 184 -23 q 0 370 0 215 q 87 634 0 522 q 330 758 183 758 q 457 728 398 758 q 564 644 515 699 l 564 737 l 686 737 l 686 34 m 582 367 q 529 560 582 481 q 358 652 468 652 q 189 561 250 652 q 135 369 135 482 q 189 176 135 255 q 361 85 251 85 q 529 176 468 85 q 582 367 582 255 " }, "²": { "x_min": 0, "x_max": 442, "ha": 539, "o": "m 442 383 l 0 383 q 91 566 0 492 q 260 668 176 617 q 354 798 354 727 q 315 875 354 845 q 227 905 277 905 q 136 869 173 905 q 99 761 99 833 l 14 761 q 82 922 14 864 q 232 974 141 974 q 379 926 316 974 q 442 797 442 878 q 351 635 442 704 q 183 539 321 611 q 92 455 92 491 l 442 455 l 442 383 " }, "–": { "x_min": 0, "x_max": 705.5625, "ha": 803, "o": "m 705 334 l 0 334 l 0 410 l 705 410 l 705 334 " }, "Κ": { "x_min": 0, "x_max": 819.5625, "ha": 893, "o": "m 819 0 l 650 0 l 294 509 l 139 356 l 139 0 l 0 0 l 0 1013 l 139 1013 l 139 526 l 626 1013 l 809 1013 l 395 600 l 819 0 " },
            "ƒ": { "x_min": -46.265625, "x_max": 392, "ha": 513, "o": "m 392 651 l 259 651 l 79 -279 l -46 -278 l 134 651 l 14 651 l 14 751 l 135 751 q 151 948 135 900 q 304 1041 185 1041 q 334 1040 319 1041 q 392 1034 348 1039 l 392 922 q 337 931 360 931 q 271 883 287 931 q 260 793 260 853 l 260 751 l 392 751 l 392 651 " },
            "e": { "x_min": 0, "x_max": 714, "ha": 813, "o": "m 714 326 l 140 326 q 200 157 140 227 q 359 87 260 87 q 488 130 431 87 q 561 245 545 174 l 697 245 q 577 48 670 123 q 358 -26 484 -26 q 97 85 195 -26 q 0 363 0 197 q 94 642 0 529 q 358 765 195 765 q 626 627 529 765 q 714 326 714 503 m 576 429 q 507 583 564 522 q 355 650 445 650 q 206 583 266 650 q 140 429 152 522 l 576 429 " },
            "ό": { "x_min": 0, "x_max": 712, "ha": 815, "o": "m 356 -25 q 94 91 194 -25 q 0 368 0 202 q 92 642 0 533 q 356 761 192 761 q 617 644 517 761 q 712 368 712 533 q 619 91 712 201 q 356 -25 520 -25 m 356 85 q 527 175 465 85 q 583 369 583 255 q 528 562 583 484 q 356 651 466 651 q 189 560 250 651 q 135 369 135 481 q 187 177 135 257 q 356 85 250 85 m 576 1040 l 387 819 l 303 819 l 438 1040 l 576 1040 " },
            "J": { "x_min": 0, "x_max": 588, "ha": 699, "o": "m 588 279 q 287 -26 588 -26 q 58 73 126 -26 q 0 327 0 158 l 133 327 q 160 172 133 227 q 288 96 198 96 q 426 171 391 96 q 449 336 449 219 l 449 1013 l 588 1013 l 588 279 " }, "»": { "x_min": -1, "x_max": 503, "ha": 601, "o": "m 503 302 l 280 136 l 281 256 l 429 373 l 281 486 l 280 608 l 503 440 l 503 302 m 221 302 l 0 136 l 0 255 l 145 372 l 0 486 l -1 608 l 221 440 l 221 302 " },
            ...
            "^": { "x_min": 193.0625, "x_max": 598.609375, "ha": 792, "o": "m 598 772 l 515 772 l 395 931 l 277 772 l 193 772 l 326 1013 l 462 1013 l 598 772 " }, "«": { "x_min": 0, "x_max": 507.203125, "ha": 604, "o": "m 506 136 l 284 302 l 284 440 l 506 608 l 507 485 l 360 371 l 506 255 l 506 136 m 222 136 l 0 302 l 0 440 l 222 608 l 221 486 l 73 373 l 222 256 l 222 136 " }, "D": { "x_min": 0, "x_max": 828, "ha": 935, "o": "m 389 1013 q 714 867 593 1013 q 828 521 828 729 q 712 161 828 309 q 382 0 587 0 l 0 0 l 0 1013 l 389 1013 m 376 124 q 607 247 523 124 q 681 510 681 355 q 607 771 681 662 q 376 896 522 896 l 139 896 l 139 124 l 376 124 " }, "∙": { "x_min": 0, "x_max": 142, "ha": 239, "o": "m 142 585 l 0 585 l 0 738 l 142 738 l 142 585 " },
            "ÿ": { "x_min": 0, "x_max": 47, "ha": 125, "o": "m 47 3 q 37 -7 47 -7 q 28 0 30 -7 q 39 -4 32 -4 q 45 3 45 -1 l 37 0 q 28 9 28 0 q 39 19 28 19 l 47 16 l 47 19 l 47 3 m 37 1 q 44 8 44 1 q 37 16 44 16 q 30 8 30 16 q 37 1 30 1 m 26 1 l 23 22 l 14 0 l 3 22 l 3 3 l 0 25 l 13 1 l 22 25 l 26 1 " },
            "w": { "x_min": 0, "x_max": 1009.71875, "ha": 1100, "o": "m 1009 738 l 783 0 l 658 0 l 501 567 l 345 0 l 222 0 l 0 738 l 130 738 l 284 174 l 432 737 l 576 738 l 721 173 l 881 737 l 1009 738 " },
            "$": { "x_min": 0, "x_max": 700, "ha": 793, "o": "m 664 717 l 542 717 q 490 825 531 785 q 381 872 450 865 l 381 551 q 620 446 540 522 q 700 241 700 370 q 618 45 700 116 q 381 -25 536 -25 l 381 -152 l 307 -152 l 307 -25 q 81 62 162 -25 q 0 297 0 149 l 124 297 q 169 146 124 204 q 307 81 215 89 l 307 441 q 80 536 148 469 q 13 725 13 603 q 96 910 13 839 q 307 982 180 982 l 307 1077 l 381 1077 l 381 982 q 574 917 494 982 q 664 717 664 845 m 307 565 l 307 872 q 187 831 233 872 q 142 724 142 791 q 180 618 142 656 q 307 565 218 580 m 381 76 q 562 237 562 96 q 517 361 562 313 q 381 423 472 409 l 381 76 " },
            "\\": { "x_min": -0.015625, "x_max": 425.0625, "ha": 522, "o": "m 425 -129 l 337 -129 l 0 1041 l 83 1041 l 425 -129 " }, "µ": { "x_min": 0, "x_max": 697.21875, "ha": 747, "o": "m 697 -4 q 629 -14 658 -14 q 498 97 513 -14 q 422 9 470 41 q 313 -23 374 -23 q 207 4 258 -23 q 119 81 156 32 l 119 -278 l 0 -278 l 0 738 l 124 738 l 124 343 q 165 173 124 246 q 308 83 216 83 q 452 178 402 83 q 493 359 493 255 l 493 738 l 617 738 l 617 214 q 623 136 617 160 q 673 92 637 92 q 697 96 684 92 l 697 -4 " },
    ...
    (function (markers) {
        ...
        function distance(marker1, marker2) {
            ...
            if (m1.group() && m2.group())
                return m1.position().distanceTo(m2.position());
            else
                return -9999;
        }
        markers.distance = distance;
        /**
         * Gets the x, y, z positional coordinates of a marker
         */
        //% blockId=ar_get_pos block="%marker=marker_block|position %axis" blockGap=8
        //% weight=99
        function position(marker, axis) {
            var m = pxsim.board().marker(marker);
            switch (axis) {
                case 0 /* x */: return m.position().x;
                case 1 /* y */: return m.position().y;
        ...
        //% weight=98
        function rotation(marker, axis) {
            var m = pxsim.board().marker(marker);
            switch (axis) {
                case 0 /* x */: return m.rotation().x;
                case 1 /* y */: return m.rotation().y;
                default: return m.rotation().z;
            }
        }
        markers.rotation = rotation;
        /**
        ...
        //% weight=96
        //% inlineInputMode="inline"    
        function mapPosition(marker, axis, out_min, out_max) {
            var m = pxsim.board().marker(marker);
            var in_min = -1.5;
            var in_max = 1.5;
        ...
        //% weight=95
        //% inlineInputMode="inline"    
        function mapRotation(marker, out_min, out_max) {
            var m = pxsim.board().marker(marker);
            var out_min_first = out_min;
            var out_min_second = out_max / 4;
            var out_min_third = out_max / 2;
            var out_min_fourth = out_max * (3 / 4);
            var out_max_first = out_max / 4;
            var out_max_second = out_max / 2;
            var out_max_third = out_max * (3 / 4);
            var out_max_fourth = out_max;
            var first_max = 40;
            var second_max = 49;
            var third_max = 25;
            var rotY = THREE.Math.radToDeg(rotation(marker, 1 /* y */));
            var w = THREE.Math.radToDeg(m.rotation().w);
            var map = 0;
            if (rotY <= 0 || w <= 0) {
                if (rotY < 0)
                    rotY *= -1;
        ...
        markers.mapRotation = mapRotation;
        /**
        ...
        //% weight=94    
        function slider(marker1, marker2, marker3) {
            // TO DO: remember last state that the slider was in if there's a flicker
            var m1 = pxsim.board().marker(marker1);
            var m2 = pxsim.board().marker(marker2);
            var m3 = pxsim.board().marker(marker3);
            var sliderVal = 0;
            var totalDist = markers.distance(m2.code(), m3.code());
            var sliderDist = markers.distance(m1.code(), m2.code());
        ...
        var Marker = (function () {
            function Marker(code, color) {
                ...
                pxsim.board().scene.add(this.paintGroup_);
            }
            Marker.prototype.triggerEvents = function () {
                /* calculate differences in previous and current positions/rotations */
                var distThreshold = 0.07;
                var angleThreshold = Math.PI / 16;
                var currentPos = this.position();
                var currentRot = this.rotation();
                var distance = this.prevPos_.distanceTo(currentPos);
                var distanceX = currentPos.x - this.prevPos_.x;
                var distanceY = currentPos.y - this.prevPos_.y;
                var distanceZ = currentPos.z - this.prevPos_.z;
                var angleX = currentRot.x - this.prevRot_.x;
                var angleY = currentRot.y - this.prevRot_.y;
                var angleZ = currentRot.z - this.prevRot_.z;
                /* trigger events depending on the changed state */
                if (distance >= distThreshold)
                    pxsim.board().bus.queue(this.code_, 3 /* Moved */);
                if (distanceX >= distThreshold)
                    pxsim.board().bus.queue(this.code_, 5 /* MovedRight */);
                else if (distanceX <= -distThreshold)
                    pxsim.board().bus.queue(this.code_, 4 /* MovedLeft */);
                if (distanceY >= distThreshold)
                    pxsim.board().bus.queue(this.code_, 6 /* MovedUp */);
                else if (distanceY <= -distThreshold)
                    pxsim.board().bus.queue(this.code_, 7 /* MovedDown */);
                if (distanceZ >= distThreshold)
                    pxsim.board().bus.queue(this.code_, 17 /* MovedForward */);
                else if (distanceZ <= -distThreshold)
                    pxsim.board().bus.queue(this.code_, 18 /* MovedBackward */);
                ...
                if (this.visible() == true) {
                    pxsim.board().bus.queue(this.code_, 20 /* WhileVisible */);
                    if (this.prevVisible_ == false)
                        pxsim.board().bus.queue(this.code_, 2 /* Visible */);
                }
                else {
                    pxsim.board().bus.queue(this.code_, 19 /* WhileHidden */);
                    if (this.prevVisible_ == true)
                        pxsim.board().bus.queue(this.code_, 1 /* Hidden */);
                }
                this.checkNeighbors();
            };
            Marker.prototype.neighbor = function (i, dist) {
                if (this.neighbors_[i][1] == dist)
                    return true;
                return false;
            };
            Marker.prototype.setNeighborDist = function (index, dist) {
                this.neighbors_[index][1] = dist;
            };
            Marker.prototype.cueTouching = function (marker, event) {
                var s = 23 /* Touching */ .toString() + 'marker' + this.code().toString() + 'marker' + marker.code().toString();
                if (event)
                    s = event + s;
                pxsim.board().bus.queue(s, 23 /* Touching */);
            };
            Marker.prototype.cueClose = function (marker, event) {
                var s = 21 /* Close */ .toString() + 'marker' + this.code().toString() + 'marker' + marker.code().toString();
                if (event)
                    s = event + s;
                pxsim.board().bus.queue(s, 21 /* Close */);
            };
            Marker.prototype.cueFar = function (marker, event) {
                var s = 22 /* Far */ .toString() + 'marker' + this.code().toString() + 'marker' + marker.code().toString();
                if (event)
                    s = event + s;
                pxsim.board().bus.queue(s, 22 /* Far */);
            };
            Marker.prototype.checkNeighbors = function () {
                for (var i = 0; i < this.neighbors_.length; i++) {
                    var marker = this.neighbors_[i][0];
                    var neighborPos = marker.position();
                    var dist = neighborPos.distanceTo(this.position());
                    if (this.code() == marker.code()) {
                        this.cueTouching(marker);
                        this.cueClose(marker);
                        if (this.neighbor(i, '') || !this.prevVisible()) {
                        ...
                        if (dist <= 2.5 && this.visible() && marker.visible()) {
                            if (dist <= 1.5) {
                                this.cueClose(marker);
                                this.cueTouching(marker);
                                if (!this.neighbor(i, 'touching') || this.neighbor(i, '')) {
                                    this.cueClose(marker, 'on');
                                    this.cueTouching(marker, 'on');
                                }
                                this.neighbors_[i][1] = 'touching';
                                ...
                                this.cueClose(marker);
                                if (!this.neighbor(i, 'close') || this.neighbor(i, '')) {
                                    this.cueClose(marker, 'on');
                                }
                            ...
                            this.cueFar(marker);
                            if (!this.neighbor(i, 'far') || this.neighbor(i, '')) {
             ...
             */
            Marker.prototype.updateState = function () {
                this.prevVisible_ = this.visible();
                this.prevPos_ = new THREE.Vector3(this.position().x, this.position().y, this.position().z);
                this.prevRot_ = this.rotation();
                this.prevWorldPos_ = new THREE.Vector3(this.worldPosition().x, this.worldPosition().y, this.worldPosition().z);
            };
            Marker.prototype.paint = function () {
                var pp = this.prevWorldPos_;
                var cp = this.worldPosition();
                if (pp != cp) {
                    var line = pxsim.three.createLine(pp, cp, this.brushColor());
                    line.name = 'markerrroot' + this.code_.toString();
                    this.paintGroup_.add(line);
                }
            };
            Marker.prototype.clearBrushStrokes = function () {
            ...
            Marker.prototype.initControls = function () {
                ...
                pxsim.board().scene.add(group);
                pxsim.board().scene.visible = false;
                var markerControls = pxsim.threex.createMarkerControls(this.code_, group);
                return group;
            };
            /* Getter methods */
            Marker.prototype.textObject = function () { return this.group_.getObjectByName(this.code_.toString() + '-text'); };
            Marker.prototype.shapeObject = function () {
            ...
            Marker.prototype.prevVisible = function () { return this.prevVisible_; };
            Marker.prototype.prevVisibleTime = function () { return this.prevVisibleTime_; };
            Marker.prototype.prevHiddenTime = function () { return this.prevHiddenTime_; };
            Marker.prototype.color = function () { return this.color_; };
            Marker.prototype.opacity = function () { return this.opacity_; };
            Marker.prototype.fontColor = function () { return this.fontColor_; };
            Marker.prototype.brushColor = function () { return this.brushColor_; };
            Marker.prototype.brushWidth = function () { return this.brushWidth_; };
            Marker.prototype.scaleX = function () { return this.scaleX_; };
            Marker.prototype.scaleY = function () { return this.scaleY_; };
            Marker.prototype.scaleZ = function () { return this.scaleZ_; };
            Marker.prototype.posX = function () { return this.posX_; };
            Marker.prototype.posY = function () { return this.posY_; };
            Marker.prototype.posZ = function () { return this.posZ_; };
            Marker.prototype.rotX = function () { return this.rotX_; };
            Marker.prototype.rotY = function () { return this.rotY_; };
            Marker.prototype.rotZ = function () { return this.rotZ_; };
            Marker.prototype.visible = function () {
                var date = new Date();
                ...
                if (this.group_) {
                    if (this.group_.visible) {
                        ...
                        if (time - this.prevVisibleTime() >= 175)
                            return false;
                        else
                            return true;
                    }
            ...
            Marker.prototype.painting = function () { return this.painting_; };
            /* Setter methods */
            Marker.prototype.setPrevVisibleTime = function (time) {
            ...
            Marker.prototype.setFontColor = function (color) {
                this.fontColor_ = color;
            };
            Marker.prototype.setBrushColor = function (color) {
                this.brushColor_ = color;
            };
            Marker.prototype.setBrushWidth = function (thickness) {
                this.brushWidth_ = thickness;
            };
            Marker.prototype.setScale = function (x, y, z) {
                this.scaleX_ = x / this.autoScale_;
                if (y)
                    this.scaleY_ = y / this.autoScale_;
                else
                    this.scaleY_ = this.scaleX_;
                if (z)
                    this.scaleZ_ = z / this.autoScale_;
                else
                    this.scaleZ_ = this.scaleX_;
            };
            Marker.prototype.setAutoScale = function (value) {
                this.autoScale_ = value;
            };
            Marker.prototype.setPosition = function (x, y, z) {
                this.posX_ = x;
                this.posY_ = y;
                this.posX_ = z;
            };
            Marker.prototype.setRotation = function (x, y, z) {
                if (x && y && z) {
            ...
            Marker.prototype.addNeighbor = function (neighbor) {
                this.neighbors_.push(neighbor);
            };
            Marker.prototype.setPainting = function (val) {
                this.painting_ = val;
            };
            return Marker;
        }());
        markers.Marker = Marker;
    })(markers = pxsim.markers || (pxsim.markers = {}));
})(pxsim || (pxsim = {}));
var pxsim;
(function (pxsim) {
    var music;
...
(function (pxsim) {
    ...
    (function (paint) {
        ...
        //% inlineInputMode="inline"    
        function setBrushMode(marker, val) {
            var m = pxsim.board().marker(marker);
            switch (val) {
                case 0 /* on */:
                    m.setPainting(true);
        ...
        paint.setBrushMode = setBrushMode;
        /**
        ...
        //% blockNamespace=paint inBasicCategory=true
        //% inlineInputMode="inline"    
        function setBrushColor(marker, color) {
            var m = pxsim.board().marker(marker);
            m.setBrushColor(color);
        }
        paint.setBrushColor = setBrushColor;
        /**
        ...
        //% blockNamespace=paint inBasicCategory=true
        //% inlineInputMode="inline"    
        function clearBrushStrokes(marker) {
            var m = pxsim.board().marker(marker);
            m.clearBrushStrokes();
        }
...
(function (pxsim) {
    ...
    var Board = (function (_super) {
        ...
        function Board() {
            _super.call(this);
            if (!isChrome()) {
                pxsim.console.log("We've detected that you are not using Chrome. " +
                    "Arcadia is currently only supported for the Chrome browser.");
            }
            this.bus = new pxsim.EventBus(pxsim.runtime);
            this.font = pxsim.three.parseFont(font.helvetiker_regular);
            /* AR */
            this.markers = {};
            ...
            this.mirror = false;
            this.vrEffect = false;
            this.camera = pxsim.three.createCamera();
            this.scene = pxsim.three.createScene();
            this.scene.add(this.camera);
            this.scene.add(pxsim.three.createDirectionalLight());
            this.scene.add(pxsim.three.createAmbientLight());
            /* music */
            this.phrases = {};
            this.instruments = [];
            this.fx = { "distortion": pxsim.tone.createEffect(1 /* Distortion */),
            ...
            this.monosynth = pxsim.tone.createMonoSynth().toMaster(); // for play tone blocks
            this.polysynth = pxsim.tone.createPolySynth(5).toMaster(); // for play chord blocks
            this.instruments.push(this.monosynth);
            this.instruments.push(this.polysynth);
            this.oscillators = { "sine": pxsim.tone.createOsc(1 /* Sine */, 440),
         ...
         */
        Board.prototype.initRenderFunctions = function () {
            var _this = this;
            this.onRenderFcts = [];
            /* update the AR toolkit source and context */
            this.onRenderFcts.push(function () {
                if (!_this.arToolkitSource || _this.arToolkitSource.ready === false)
                    return;
            ...
            this.onRenderFcts.push(function () {
                if (!_this.vrEffect) {
                    ...
                    if (_this.stereoRenderer && _this.scene && _this.camera)
                        _this.stereoRenderer.render(_this.scene, _this.camera);
                }
            });
            /* updates marker state and triggers events if position or rotation changes */
            this.onRenderFcts.push(function () {
                for (var key in _this.markers) {
                    var marker = _this.markers[key];
        ...
        Board.prototype.initMarkers = function () {
            for (var i = 0; i < 16; i++) {
                //this.marker(i);
                pxsim.design.setShape(i, 0 /* Box */);
            }
        };
        Board.prototype.runRenderingLoop = function () {
            var self = this;
            var lastTimeMsec = 0;
         ...
         * @param marker
         */
        Board.prototype.marker = function (marker) {
            if (!this.markers)
                this.markers = {};
            var m = this.markers[marker.toString()];
            if (!m)
    ...
    var stereoRenderer = null;
    function getStereoRenderer() {
        if (stereoRenderer == null) {
            //stereoRenderer = new (THREE as any).StereoEffect(getWebGlContext());
            stereoRenderer = new THREE.VREffect(getWebGlContext());
            var width = window.innerWidth ||
                document.documentElement.clientWidth ||
                document.body.clientWidth ||
                document.body.offsetWidth;
...
(function (pxsim) {
    ...
    (function (three) {
        ...
        three.createCamera = createCamera;
        /**
        ...
        three.createDirectionalLight = createDirectionalLight;
        /**
        ...
        three.createAmbientLight = createAmbientLight;
        /**
        ...
        three.createGeometry = createGeometry;
        /**
        ...
        three.createBox = createBox;
        function createSphere() {
            return new THREE.SphereGeometry(0.5, 32, 32);
        }
        three.createSphere = createSphere;
        function createCone() {
            return new THREE.ConeGeometry(0.5, 1, 32);
        }
        three.createCone = createCone;
        function createCylinder() {
            return new THREE.CylinderGeometry(0.5, 0.5, 1, 32);
        }
        three.createCylinder = createCylinder;
        function createTetrahedron() {
            return new THREE.TetrahedronGeometry(1, 0);
        }
        three.createTetrahedron = createTetrahedron;
        function createIcosahedron() {
            return new THREE.IcosahedronGeometry(0.5, 0);
        }
        three.createIcosahedron = createIcosahedron;
        function createPlane() {
            return new THREE.PlaneGeometry(1, 1, 32);
        }
        three.createPlane = createPlane;
        ...
        three.createLine = createLine;
        /**
         * Creates a solid colored plane in a particular marker group
         * @param marker
         * @param color
         */
        function createBillboard(marker, color) {
            var m = pxsim.board().marker(marker);
        ...
        three.createBillboard = createBillboard;
        function createText(text, color, marker) {
        ...
        three.createText = createText;
        function removeTextFromMarker(marker) {
            var m = pxsim.board().marker(marker);
            var textObject = m.textObject();
            if (textObject)
                three.removeObjectFromGroup(m.group(), textObject);
        }
        three.removeTextFromMarker = removeTextFromMarker;
        function removeShapeFromMarker(marker) {
            var m = pxsim.board().marker(marker);
            var shapeObject = m.shapeObject();
            if (shapeObject)
                three.removeObjectFromGroup(m.group(), shapeObject);
        }
        three.removeShapeFromMarker = removeShapeFromMarker;
        /**
         * Remove a particular object by name from a group
         * @param group
         * @param obj
         */
        function removeObjectFromGroup(group, obj) {
            group.remove(obj);
        }
        three.removeObjectFromGroup = removeObjectFromGroup;
        /**
         * Removes all objects and child objects from scene
         * @param scene
         */
        function removeSceneChildren(scene) {
            while (scene.children.length) {
                scene.remove(scene.children[0]);
            }
        }
        three.removeSceneChildren = removeSceneChildren;
        function loadFontAsync(url) {
            var loader = new THREE.FontLoader();
            return new Promise(function (resolve, reject) {
                loader.load(url + "fonts/helvetiker_regular.typeface.json", function (font) {
                    resolve(font);
                }, null, function (e) { return reject(e); });
            });
        }
        three.loadFontAsync = loadFontAsync;
        function parseFont(typeface) {
            var loader = new THREE.FontLoader();
            var font = loader.parse(typeface);
            return font;
        }
        three.parseFont = parseFont;
        function loadModel(type, content) {
            var loader = null;
            switch (type) {
                case 1 /* STL */:
            ...
            var model = loader.parse(content);
            return model;
        }
        three.loadModel = loadModel;
    })(three = pxsim.three || (pxsim.three = {}));
})(pxsim || (pxsim = {}));
var pxsim;
(function (pxsim) {
    var threex;
    (function (threex) {
        function createMarkerControls(marker, markerRoot) {
            return new THREEx.ArMarkerControls(pxsim.board().arToolkitContext, markerRoot, {
                type: 'barcode',
                barcodeValue: marker,
        ...
        threex.createMarkerControls = createMarkerControls;
        var arToolkitContextSingleton = null;
        var arToolkitSourceSingleton = null;
        function initArToolkit() {
            if (!arToolkitContextSingleton && !arToolkitSourceSingleton) {
                ...
                var arToolkitContext_1 = new THREEx.ArToolkitContext({
                    cameraParametersUrl: pxsim.board().baseURL + 'AR.js/data/data/camera_para.dat',
                    detectionMode: 'mono_and_matrix',
                    matrixCodeType: '3x3'
                });
                var self_1 = pxsim.board();
                arToolkitSource_1.init(function onReady() {
                    onResize();
                });
                ...
                arToolkitContext_1.init(function onCompleted() {
                    if (self_1.camera)
                        self_1.camera.projectionMatrix.copy(arToolkitContext_1.getProjectionMatrix());
                });
                arToolkitContextSingleton = arToolkitContext_1;
                arToolkitSourceSingleton = arToolkitSource_1;
            }
            pxsim.board().arToolkitContext = arToolkitContextSingleton;
            pxsim.board().arToolkitSource = arToolkitSourceSingleton;
        }
